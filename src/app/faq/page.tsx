...
function FAQContent() {
  ...
  const categories = useMemo(() => [
    ...
    { id: "overview", labelEn: "Overview & Scope", labelHi: "पोर्टल परिचय एवं उद्देश्य", indices: [0, 1, 2] },
    { id: "channels", labelEn: "Channels & Filing", labelHi: "शिकायत दर्ज करने के माध्यम", indices: [3, 4] },
    { id: "tracking", labelEn: "Tracking & Acknowledgement", labelHi: "पावती एवं ट्रैकिंग", indices: [5, 6, 12] },
    { id: "sla", labelEn: "SLA & Escalation", labelHi: "समय सीमा एवं एस्केलेशन", indices: [7, 8, 9, 11] },
    { id: "governance", labelEn: "Governance & Support", labelHi: "सुशासन एवं सहायता", indices: [10, 13, 14] },
  ], []);
    ...
    return t.faqs.items
      ...
      .filter((item) => {
        ...
        if (activeCategory !== "all") {
          ...
          if (category && category.indices && !category.indices.includes(item.originalIndex)) {
            return false;
          }
        }
        return true;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 font-sans selection:bg-[#1C4D8D] selection:text-white">
      ...
      {/* 3. Main Navbar */}
      <MainNavbar onOpenModal={(modal) => setActiveModal(modal)} />
      ...
      <main className="flex-1">
        ...
        <div className="bg-slate-100 border-b border-slate-200 py-3 px-4 sm:px-8">
          <div className="max-w-5xl mx-auto flex items-center gap-2 text-xs text-slate-500 font-medium">
            <Link href="/" className="hover:text-[#1C4D8D] flex items-center gap-1">
        ...
        <section className="py-10 px-4 sm:px-8">
          <div className="max-w-5xl mx-auto">
            ...
            {/* Filter Category Pills & Controls */}
            <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 mb-8 pb-4 border-b border-slate-200">
              ...
              <div className="flex items-center gap-2 self-end md:self-auto shrink-0 text-xs">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 font-semibold transition-colors cursor-pointer"
                >
            ...
            {/* Questions List with Solution Dropdowns */}
            {filteredFaqs.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-3xl border border-slate-200 shadow-xs">
                ...
                <h3 className="text-base font-bold text-slate-800">
                  {lang === "hi" ? "कोई प्रश्न नहीं मिला" : "No matching questions found"}
                </h3>
              ...
              <div className="space-y-3.5">
                {filteredFaqs.map((item) => {
                  const isOpen = !!expandedFaqs[item.originalIndex];
                  ...
                  return (
                    ...
                    >
                      {/* Question Header (Normal Question) */}
                      <button
                        type="button"
                            ...
                            {item.answer.split("\n").map((line, lIdx) => {
                              if (line.startsWith("•")) {
                                return (
                                  <div
                                    key={lIdx}
                                    className="flex items-start gap-2 py-0.5 text-slate-800 font-medium pl-2"
                                  >
            ...
            <div className="mt-12 bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xs">
              ...
              <div className="flex flex-wrap items-center gap-3 shrink-0">
                ...
                >
                  <span>{lang === "hi" ? "शिकायत दर्ज करें" : "File Complaint"}</span>
